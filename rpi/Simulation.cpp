...
static char*  ReadFormattedString(  FILE* Fptr, double* numberToReturnToCallingFunction );
...
static void   CloseOutputFiles( char* errorMessage, int isPrintToScreen, int isPrintToFile );
...
{
   double tInitial, tFinal, tStep, absError, relError, printIntScreenDbl, printIntFileDbl;
   ...
   CloseOutputFiles( errorMessage, (int)printIntScreenDbl, (int)printIntFileDbl );
...
static char*  OpenOutputFilesWriteHeaders( int isPrintToScreen, int isPrintToFile )
{
   ...
   if( !myHasHeaderInfoBeenWritten && (isPrintToScreen || isPrintToFile) )
   {
      ...
      {
         char* outputFileName = "Projet.1";
         myFptr[0] = fopen( outputFileName, "w" );
         if( myFptr[0] ) fputs( headerFile[0], myFptr[0] );
         else { errorMessage = "Error: Unable to open/write to file";  printf("%s %s\n", errorMessage, outputFileName); }
      }
...
static void  CloseOutputFiles( char* errorMessage, int isPrintToScreen, int isPrintToFile )
{
   if( errorMessage )
   {
      int iMin = (isPrintToScreen ? -1 : 0),  iMax = (isPrintToFile && myFptr[0]) ? 1 : 0,  i;
      for( i = iMin;  i < iMax;  i++ )  fprintf( i==-1 ? stdout : myFptr[i], "\n%% %s\n", errorMessage );
   }

   if( myFptr[0] ) fclose( myFptr[0] );  myFptr[0] = NULL;
   if( isPrintToScreen )
   {
